...
class AlexNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.features = nn.Sequential(
            # 第1层：缩小卷积核（原11x11→5x5），减少通道数（原96→64）
            nn.Conv2d(3, 64, kernel_size=5, stride=1, padding=2),  # 32x32x3 → 32x32x64
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),  # 32x32x64 → 15x15x64
            nn.LocalResponseNorm(size=5),  # 局部响应归一化

            # 第2层：减少通道数（原256→192），避免过拟合
            nn.Conv2d(64, 192, kernel_size=5, stride=1, padding=2),  # 15x15x64 → 15x15x192
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),  # 15x15x192 → 7x7x192

            # 第3层：保留基础结构，大幅减少通道（原384→384→128）
            nn.Conv2d(192, 384, kernel_size=3, padding=1),  # 7x7x192 → 7x7x384
            nn.ReLU(inplace=True),
            nn.Conv2d(384, 256, kernel_size=3, padding=1),  # 7x7x384 → 7x7x256
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 128, kernel_size=3, padding=1),  # 7x7x256 → 7x7x128
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),  # 7x7x128 → 3x3x128
        )

        # 全连接层进行一定程度的压缩
        self.classifier = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(3 * 3 * 128, 1024),  # 3*3*128 → 1024
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
            nn.Linear(1024, 512),  # 1024 → 512
            nn.ReLU(inplace=True),
            nn.Linear(512, num_classes)  # 输出10类
        )
...
'''
结果评估：
30轮训练后测试集准确率72.5%左右，相比之前的几个模型提升显著
这证明了把CNN做深做宽，能够显著提升模型性能
测试集精度和训练集精度相当，说明模型泛化能力较好，这也离不开dropout、weight decay等正则化手段的帮助
'''