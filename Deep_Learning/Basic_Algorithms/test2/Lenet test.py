...
if __name__ == '__main__':
    ...
    # 定义Lenet模型
    model = nn.Sequential(
        nn.Conv2d(1, 6, kernel_size=5, padding=2), nn.Tanh(),  # 28x28x1 -> 28x28x6
        nn.AvgPool2d(kernel_size=2, stride=2),  # 28x28x6 -> 14x14x6
        nn.Conv2d(6, 16, kernel_size=5), nn.Tanh(),  # 14x14x6 -> 10x10x16
        nn.AvgPool2d(kernel_size=2, stride=2),  # 10x10x16 -> 5x5x16
        nn.Flatten(),  # 5x5x16 -> 16*5*5
        nn.Linear(16 * 5 * 5, 120), nn.Tanh(),  # 16*5*5 -> 120
        nn.Linear(120, 84), nn.Tanh(),  # 120 -> 84
